...
export default function VisitDetail() {
  ...
  useEffect(() => {
    ...
    api.request(`/visits/${id}`)
      .then((data) => { if (isMounted) setVisit(data); })
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center text-slate-400 gap-3">
        ...
        <span className="text-xs font-bold uppercase tracking-wider">Buscando histórico do chamado...</span>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white border border-slate-200 rounded-xl p-6 text-center max-w-sm w-full shadow-xs">
          <p className="text-rose-600 font-semibold mb-4">{error || "Visita não encontrada."}</p>
          <button onClick={() => navigate("/dashboard")} className="w-full h-10 bg-indigo-600 text-white rounded-lg text-xs font-bold cursor-pointer">
            Voltar ao Dashboard
          </button>
        </div>
      </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-4 border-b border-slate-100">
              <div>
                ...
                <h2 className="text-xl font-black text-slate-900 mt-1">{visit.client_name}</h2>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Local da Execução</label>
                <p className="font-semibold text-slate-700">📍 {visit.address}</p>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Técnico Alocado</label>
                <p className="font-semibold text-slate-700">👷‍♂️ {visit.technician?.name}</p>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Janela de Atendimento</label>
                <p className="font-semibold text-slate-700">
                  📅 {new Date(visit.scheduled_at).toLocaleString("pt-BR")}
                </p>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Última Atualização</label>
            ...
            {visit.attachments.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {visit.attachments.map((img) => (
                  <div key={img.id} className="group relative aspect-square bg-slate-100 border border-slate-200 rounded-xl overflow-hidden shadow-xs hover:border-indigo-500 transition-colors">
                    <img 
